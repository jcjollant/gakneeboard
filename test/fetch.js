...
function readAdip( code) {
    ...
    .then( response => {
        ...
        let airport = fromAdip( response.data)
...
function fromAdip( adip) {
    ...
    var uniqueCode = ''
    if( 'icaoId' in adip) {
        uniqueCode = adip.icaoId.toLowerCase()
    } else if('locId' in adip){
        uniqueCode = adip.locId.toLowerCase()
    } else {
        console.error('No unique code found')
    }
