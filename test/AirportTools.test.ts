...
describe( 'Airport Tools', () => {
    test( 'Renton v5 to v6', () => {
        const view = AirportTools.format(rentonV5)
        expect(view).toBeInstanceOf(AirportView)
        if( view) {
            expect(view.code).toBe('KRNT')
            expect(view.name).toBe('Renton Muni')
            expect(view.freq.length).toBe(3)
            expect(view.freq[0].name).toBe('CTAF')
            expect(view.freq[0].mhz).toBe(124.7)
            expect(view.freq[1].name).toBe('GND')
            expect(view.freq[1].mhz).toBe(121.6)
            expect(view.freq[2].name).toBe('ATIS')
            expect(view.freq[2].mhz).toBe(126.95)
    ...
    test('Runway frequency', () => {
        const view = AirportTools.format(boeingV5)
        // console.log(v6)
        expect(view).toBeInstanceOf(AirportView)
        if( view) {
            ...
            const rwy0Freq =  AirportTools.getFrequency(view.freq, "RWY " + rwy0Name)
