...
describe('navlog Tools', () => {
    ...
    test('Flight Plan Import', async () => {
        // Mock AirportService for invalid KBVS.fpl airports
        // The original test expects KRNT alt 32 and KBVS alt 145.1
        const mockAirportService = AirportService as unknown as jest.Mocked<typeof AirportService>;
        mockAirportService.getAirport.mockImplementation(async (code) => {
            if (code === 'KRNT') return { elev: 32 } as any;
            if (code === 'KBVS') return { elev: 145.1 } as any;
            return undefined;
        });

        const buffer = fs.readFileSync('./test/KBVS.fpl')
        await NavlogTools.importFlightPlan(buffer).then(list => {
            const expected = [
