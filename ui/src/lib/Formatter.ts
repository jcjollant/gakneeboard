...
export class Formatter {
    ...
    static feet(length: any): string {
        if (length == null || length === undefined || (typeof length === 'number' && isNaN(length))) return '?';
        let value = typeof length === 'string' ? Number(length) : length;
