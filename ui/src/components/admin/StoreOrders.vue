<template>
    <div class="store-orders">
        ...
        <div v-else-if="orders.length === 0" class="empty">No processing orders found.</div>
...
const fetchOrders = async () => {
    loading.value = true;
    try {
        const res = await currentUser.getUrl(UrlService.root + 'admin/orders');
...
const markShipped = async (orderId: string) => {
    ...
    try {
        await currentUser.postUrl(UrlService.root + 'admin/orders/' + orderId + '/ship');
